...
//                }
    ...
    func trainCompareHealthOnDevice(userData: [UserData], target: HKQuantityTypeIdentifier, target2: HKQuantityTypeIdentifier, completionHandler: @escaping (ModelResponse) -> Void) {
        ...
        let filteredToTarget = filteredToRemoveNan.filter { data in
            return data.title == target.rawValue
        }
        
        let filteredToTarget2 = filteredToRemoveNan.filter { data in
            return data.title == target2.rawValue
        }
        ...
        var column = Column<Double>(name: target.rawValue, capacity: smallestCount)
        column.append(contentsOf: dataArray)
       
        var column2 = Column<Double>(name: target2.rawValue, capacity: smallestCount)
        ...
        do {
           ...
           var doubleArray = [Double]()
            ...
            completionHandler(ModelResponse(type: target.rawValue, predicted: predictions.map{($0.unsafelyUnwrapped) as! Double}, actual: doubleArray, accuracy: model.trainingMetrics.rootMeanSquaredError))
        } catch {
            print(error)
    ...
    func trainOnDevice(userData: [UserData], target: DataType, completionHandler: @escaping () -> Void) {
        var trainingData = DataFrame()
        let filteredToRemoveNan = userData.filter { data in
            return data.data.isNormal && !data.data.isNaN
        }
        ...
        for type in DataType.allCases {
            ...
            if !filteredToType.isEmpty {
                ...
                if filteredToType.count > 1000 {
                    for i in 0...filteredToType.count - 1000 {
